(function () {
  ...
  function createRoom(names, roomCode) {
    ...
    return {
      ...
      events: seededOrder(game.events, roomCode).map(event => event.id)
    };
  }
  ...
  function getEvent(room) {
    ...
    return game.events.find(event => event.id === id);
  }
  ...
  function settleRound(room) {
    ...
    room.players.forEach(player => {
      ...
      player.history.push({ round: room.round + 1, before, after: player.money, weightedReturn, strategy: player.choice.id });
      player.choice = null;
    });
