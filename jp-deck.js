/* ══════════════════════════════════════════════════════════════════════
   jp-deck.js — 제이피매쓰랩 수업자료 공용 런타임
   ----------------------------------------------------------------------
   ① 교사용 비밀번호 게이트 (#teacherGate 가 있을 때만 동작)
   ② html-ppt runtime : ← → 이동 / F 전체화면 / S 교사용 창 / N 노트 /
                        O 한눈에 보기 / T 테마 / URL #/N 딥링크
   ③ 하단 크롬 동기화 · 터치 네비 · 노트 드로어
   ══════════════════════════════════════════════════════════════════════ */

/* ─────────── ① 교사용 게이트 ─────────── */
(function () {
  if (!document.getElementById('teacherGate')) return;
  (function () {
    const ACCESS_KEY = 'jp-math-teacher-access';
...
(function () {
  ...
  (function(){
    ...
    function sync(){
      ...
      if(i<0) return;
      now.textContent=i+1;
      chap.textContent=slides[i].getAttribute('data-title')||'';
      document.body.classList.toggle('on-cover', slides[i].classList.contains('slide--title'));
    }
  ...
  document.addEventListener('click',e=>{
    const ov=document.querySelector('.notes-overlay');
    if(!ov||!ov.classList.contains('open')) return;
    if(ov.contains(e.target)||e.target.closest('#touchNotes')) return;
