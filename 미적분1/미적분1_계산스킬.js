(function(){
  ...
  const initialBossStateLabel=bossConfig?.mechanic==='factor-shield'?'SHIELD':bossConfig?.mechanic==='conjugate-reflect'?'MIRROR':bossConfig?.mechanic==='degree-grade'?'GRADE':bossConfig?.mechanic==='chaos-split'?'BODIES':bossConfig?.mechanic==='side-switch'?'방향':bossConfig?.mechanic==='continuity-stitch'?'봉합':'PHASE';
  const initialBossStateValue=bossConfig?.mechanic==='factor-shield'?3:bossConfig?.mechanic==='conjugate-reflect'?'0/2':bossConfig?.mechanic==='degree-grade'?'C':bossConfig?.mechanic==='side-switch'?'← 좌':bossConfig?.mechanic==='continuity-stitch'?'0/3':1;
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch';$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':gradeBattle?'GRADE':chaosBattle?'BODIES':sideBattle?'방향':stitchBattle?'봉합':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:gradeBattle?boss.attackGrade:chaosBattle?boss.splitCount:sideBattle?(boss.targetSide==='right'?'우 →':'← 좌'):stitchBattle?`${boss.stitches}/3`:boss.phase;
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,questionStartedAt:0,q:null,locked:false};
  ...
  function renderBossV2(){
    if(!boss.running)return;boss.locked=false;const level=bossQuestionLevel(),sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch';if(stitchBattle&&boss.reopenOnNext){boss.stitches=0;boss.reopenOnNext=false;boss.regenBuffer=0}$('[data-boss-stage]').classList.remove('boss-stitch-complete');boss.q=makeQuestion(skill.id,level,sideBattle?boss.nextSide:null);if(sideBattle)boss.targetSide=boss.q.side||boss.nextSide;boss.attackGrade=level==='deep'?'A':level==='applied'?'B':'C';boss.questionStartedAt=performance.now();const levelName=LEVELS.find(l=>l.id===level).name,directionCue=sideBattle?` · ${boss.targetSide==='left'?'← 좌극한':'우극한 →'}`:stitchBattle?` · 금실 ${boss.stitches}/3`:'';const body=$('[data-boss-body]');body.innerHTML=`<div class="question-topline"><span>PHASE ${boss.phase} · ${levelName} 문제${directionCue}</span><span data-live-combo></span></div><div class="question-equation"></div><div class="question-prompt"></div><div class="answer-grid"></div>`;
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',breakingShield=shieldBattle&&boss.shield>0,reflecting=reflectBattle&&boss.reflectCharge===1,chaosLimit=boss.phase===3?3.5:boss.phase===2?4.1:4.8,fastFusion=chaosBattle&&answerSeconds<=chaosLimit,boundaryCrossing=sideBattle&&boss.lastSide&&boss.lastSide!==q.side,completingStitch=stitchBattle&&boss.stitches===2;
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,questionStartedAt:0,q:null,locked:false}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
