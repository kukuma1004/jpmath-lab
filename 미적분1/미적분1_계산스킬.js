(function(){
  ...
  const initialBossStateLabel=bossConfig?.mechanic==='factor-shield'?'SHIELD':bossConfig?.mechanic==='conjugate-reflect'?'MIRROR':bossConfig?.mechanic==='degree-grade'?'GRADE':bossConfig?.mechanic==='chaos-split'?'BODIES':bossConfig?.mechanic==='side-switch'?'방향':bossConfig?.mechanic==='continuity-stitch'?'봉합':bossConfig?.mechanic==='squeeze-walls'?'벽 간격':bossConfig?.mechanic==='forbidden-seal'?'정석 봉인':'PHASE';
  const initialBossStateValue=bossConfig?.mechanic==='factor-shield'?3:bossConfig?.mechanic==='conjugate-reflect'?'0/2':bossConfig?.mechanic==='degree-grade'?'C':bossConfig?.mechanic==='side-switch'?'← 좌':bossConfig?.mechanic==='continuity-stitch'?'0/3':bossConfig?.mechanic==='squeeze-walls'?'100%':bossConfig?.mechanic==='forbidden-seal'?'3/3':1;
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',wallGap=Number.isFinite(boss.wallGap)?boss.wallGap:100;$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':gradeBattle?'GRADE':chaosBattle?'BODIES':sideBattle?'방향':stitchBattle?'봉합':squeezeBattle?'벽 간격':forbiddenBattle?'정석 봉인':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:gradeBattle?boss.attackGrade:chaosBattle?boss.splitCount:sideBattle?(boss.targetSide==='right'?'우 →':'← 좌'):stitchBattle?`${boss.stitches}/3`:squeezeBattle?`${Math.round(wallGap)}%`:forbiddenBattle?`${boss.orthodoxSeal}/3`:boss.phase;
  ...
  function renderBossV2(){
    if(!boss.running)return;boss.locked=false;const level=bossQuestionLevel(),sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal';if(stitchBattle&&boss.reopenOnNext){boss.stitches=0;boss.reopenOnNext=false;boss.regenBuffer=0}if(squeezeBattle){boss.wallGap=100;boss.wallCrushed=false}if(forbiddenBattle&&boss.resealOnNext){boss.orthodoxSeal=3;boss.resealOnNext=false}$('[data-boss-stage]').classList.remove('boss-stitch-complete','boss-wall-slam','boss-seal-break','boss-forbidden-cast');boss.q=forbiddenBattle?makeForbiddenQuestion(level):makeQuestion(skill.id,level,sideBattle?boss.nextSide:null);if(sideBattle)boss.targetSide=boss.q.side||boss.nextSide;boss.attackGrade=level==='deep'?'A':level==='applied'?'B':'C';boss.questionStartedAt=performance.now();const levelName=LEVELS.find(l=>l.id===level).name,directionCue=sideBattle?` · ${boss.targetSide==='left'?'← 좌극한':'우극한 →'}`:stitchBattle?` · 금실 ${boss.stitches}/3`:squeezeBattle?` · 압착 ${squeezeSafeWindow().toFixed(1)}초`:forbiddenBattle?` · 정석 ${3-boss.orthodoxSeal}/3`:'';const body=$('[data-boss-body]');body.innerHTML=`<div class="question-topline"><span>PHASE ${boss.phase} · ${levelName} 문제${directionCue}</span><span data-live-combo></span></div><div class="question-equation"></div><div class="question-prompt"></div><div class="answer-grid"></div>`;
