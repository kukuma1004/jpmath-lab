(function(){
  ...
  const LEVELS=[
    ...
    {id:'applied', name:'응용', tag:'APPLIED', desc:'식의 위치·분모·조건이 달라집니다. 핵심 변형을 먼저 골라야 합니다.'},
    {id:'deep', name:'심화', tag:'DEEP', desc:'풀이 방법·접근 방향·부호·역조건까지 함께 판단합니다.'}
  ];
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse',wallGap=Number.isFinite(boss.wallGap)?boss.wallGap:100;$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':gradeBattle?'GRADE':chaosBattle?'BODIES':sideBattle?'방향':stitchBattle?'봉합':squeezeBattle?'벽 간격':forbiddenBattle?'정석 봉인':differenceBattle?'h 거리':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:gradeBattle?boss.attackGrade:chaosBattle?boss.splitCount:sideBattle?(boss.targetSide==='right'?'우 →':'← 좌'):stitchBattle?`${boss.stitches}/3`:squeezeBattle?`${Math.round(wallGap)}%`:forbiddenBattle?`${boss.orthodoxSeal}/3`:differenceBattle?hDistanceLabel():boss.phase;
  ...
  function makeDifferenceQuestion(level){
    ...
    a=nonzero(-3,3);b=nonzero(-4,4);correct=2*a+b;q=Q('h 수축 · 이차함수',`lim h→0  (f(${num(a)}+h)−f(${num(a)}))/h`,`f(x)=x²${tail(b,'x')}일 때 차분몫의 극한값은?`,correct,[a+b,2*a-b,a*a+b],`분자를 전개하면 h(${2*a+b}+h)이고 h를 약분한 뒤 h→0을 적용하면 ${correct}입니다.`);q.hMode='이차';return q
  }
  function makeForbiddenQuestion(level){
    let q,a,n,c;
    if(level==='deep'){
      a=ri(2,5);q=Q('정석 봉인 · 통분',`lim x→${a}  (1/x−1/${a})/(x−${a})`,'학교 시험 풀이에서 가장 먼저 사용할 방법은?','통분',['로피탈 정리','분자·분모 따로 대입','미분계수 공식'],`분자 1/x−1/${a}를 통분하면 (${a}−x)/(${a}x)입니다. 그다음 x−${a}를 약분하는 교육과정 안의 풀이가 먼저입니다.`);q.sealMethod='통분';return q
    }
    if(level==='applied'){
      a=ri(1,4);c=ri(1,5);q=Q('정석 봉인 · 유리화',`lim x→${a}  (√(x+${c})−√${a+c})/(x−${a})`,'학교 시험 풀이에서 가장 먼저 사용할 방법은?','유리화',['로피탈 정리','최고차항 비교','분자·분모 따로 대입'],`분자에 켤레식 √(x+${c})+√${a+c}을 곱해 x−${a}를 만든 뒤 약분합니다. 로피탈은 검산에만 사용합니다.`);q.sealMethod='유리화';return q
    }
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit(),startPhase=bossStartPhase();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:startPhase,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,wallGap:100,wallCrushed:false,wallEscapes:0,orthodoxSeal:3,forbiddenPower:0,sealBreaks:0,resealOnNext:false,lastForbidden:false,hStep:0,hCoreHits:0,hResetOnNext:false,questionStartedAt:0,q:null,locked:false};
  ...
  function renderBossV2(){
    if(!boss.running)return;boss.locked=false;const level=bossQuestionLevel(),sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse';if(stitchBattle&&boss.reopenOnNext){boss.stitches=0;boss.reopenOnNext=false;boss.regenBuffer=0}if(squeezeBattle){boss.wallGap=100;boss.wallCrushed=false}if(forbiddenBattle&&boss.resealOnNext){boss.orthodoxSeal=3;boss.resealOnNext=false}if(differenceBattle&&boss.hResetOnNext){boss.hStep=0;boss.hResetOnNext=false}$('[data-boss-stage]').classList.remove('boss-stitch-complete','boss-wall-slam','boss-seal-break','boss-forbidden-cast','boss-h-collapse','boss-h-rebound');boss.q=forbiddenBattle?makeForbiddenQuestion(level):differenceBattle?makeDifferenceQuestion(level):makeQuestion(skill.id,level,sideBattle?boss.nextSide:null);if(sideBattle)boss.targetSide=boss.q.side||boss.nextSide;boss.attackGrade=level==='deep'?'A':level==='applied'?'B':'C';boss.questionStartedAt=performance.now();const levelName=LEVELS.find(l=>l.id===level).name,directionCue=sideBattle?` · ${boss.targetSide==='left'?'← 좌극한':'우극한 →'}`:stitchBattle?` · 금실 ${boss.stitches}/3`:squeezeBattle?` · 압착 ${squeezeSafeWindow().toFixed(1)}초`:forbiddenBattle?` · 정석 ${3-boss.orthodoxSeal}/3`:differenceBattle?` · h=${hDistanceLabel()}`:'';const body=$('[data-boss-body]');body.innerHTML=`<div class="question-topline"><span>PHASE ${boss.phase} · ${levelName} 문제${directionCue}</span><span data-live-combo></span></div><div class="question-equation"></div><div class="question-prompt"></div><div class="answer-grid"></div>`;
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:bossStartPhase(),shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,wallGap:100,wallCrushed:false,wallEscapes:0,orthodoxSeal:3,forbiddenPower:0,sealBreaks:0,resealOnNext:false,lastForbidden:false,hStep:0,hCoreHits:0,hResetOnNext:false,questionStartedAt:0,q:null,locked:false}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
