(function(){
  ...
  app.innerHTML=`<div class="calc-skill-page">
    ...
    <main class="skill-wrap">
      ...
      <section class="hidden" data-panel="boss"><header class="mode-header"><div><p class="skill-kicker">${bossV2?'BOSS BATTLE 2.2':'TIMED BOSS BATTLE'}</p><h1>${skill.title} 보스전</h1><p>${bossIntro}</p></div><div class="mode-stat"><span>CLEARS</span><strong data-boss-clears>0</strong></div></header><div class="boss-stage${bossV2?` boss-v2 boss-${bossConfig.theme}`:''}"${bossV2&&bossConfig.accent?` style="--boss-accent:${bossConfig.accent}"`:''} data-boss-stage><div class="boss-arena"><div class="boss-fx-layer" data-boss-fx aria-hidden="true"></div><div class="boss-monster" data-boss-monster aria-label="${bossName}"><div class="boss-aura"></div>${bossVisual}<strong>${bossName}</strong><span data-boss-mood>도전자를 기다리는 중</span></div><div class="boss-dashboard"><div class="boss-hp-label"><span>BOSS HP</span><strong data-boss-hp-text>${bossV2?`${bossMaxHp()} / ${bossMaxHp()}`:'3 / 3'}</strong></div><div class="boss-hp-track"><div data-boss-hp></div></div><div class="boss-clock"><span>남은 시간</span><strong data-boss-time>${bossV2?`${bossBaseTime()}.0`:'12.0'}</strong><em>SEC</em></div><div class="boss-timer-track"><div data-boss-timer></div></div>${bossV2?`<div class="boss-combat-stats"><div><span>DAMAGE</span><b data-boss-damage>0</b></div><div><span>COMBO</span><b data-boss-combo>×1.0</b></div><div><span data-boss-state-label>${initialBossStateLabel}</span><b data-boss-phase>${initialBossStateValue}</b></div></div><small class="boss-limit-copy">${bossDifficultyCopy}</small>`:'<small class="boss-limit-copy">현재 문제당 <b data-boss-limit>12.0초</b> · 최소 7초</small>'}</div></div><div class="boss-progress"><i class="boss-node"></i><i class="boss-node"></i><i class="boss-node"></i></div><div data-boss-body><div class="boss-start-copy">${bossStartCopy()}</div></div><div class="boss-status hidden" data-boss-status></div><button class="primary-action boss-action" data-boss-start>⚔ ${bossV2?`${bossBaseTime()}초 전투 시작`:'보스전 시작'}</button></div></section>
    </main></div>`;
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse',productBattle=bossConfig.mechanic==='product-blades',sniperBattle=bossConfig.mechanic==='sniper-lock',stepBattle=bossConfig.mechanic==='step-lock',wallGap=Number.isFinite(boss.wallGap)?boss.wallGap:100;$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':gradeBattle?'GRADE':chaosBattle?'BODIES':sideBattle?'방향':stitchBattle?'봉합':squeezeBattle?'벽 간격':forbiddenBattle?'정석 봉인':differenceBattle?'h 거리':productBattle?'쌍날':sniperBattle?'조준':stepBattle?bossConfig.lockLabel:'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:gradeBattle?boss.attackGrade:chaosBattle?boss.splitCount:sideBattle?(boss.targetSide==='right'?'우 →':'← 좌'):stitchBattle?`${boss.stitches}/3`:squeezeBattle?`${Math.round(wallGap)}%`:forbiddenBattle?`${boss.orthodoxSeal}/3`:differenceBattle?hDistanceLabel():productBattle?bladeStateLabel():sniperBattle?sniperLockLabel():stepBattle?stepLockLabel():boss.phase;
  ...
  function stepLockLabel(){return `${boss.lockStep||0}/${bossConfig.lockSteps||3}`}
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){
      ...
      if(stepBattle){if(finishingStep){boss.lockResetOnNext=true;$('[data-boss-stage]').classList.add('boss-step-finish')}else boss.lockStep=Math.min((bossConfig.lockSteps||3)-1,(boss.lockStep||0)+1)}
