(function(){
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect';$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:boss.phase;
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,q:null,locked:false};
    ...
    const monster=$('[data-boss-monster]');monster.classList.remove('boss-hit','boss-attack','boss-defeated');$('[data-boss-mood]').textContent=bossConfig.mechanic==='factor-shield'?'석문 보호막 3칸':bossConfig.mechanic==='conjugate-reflect'?'거울 충전 0 / 2':'기본 방어 · PHASE 1';$('[data-boss-start]').classList.add('hidden');$('[data-boss-status]').className='boss-status hidden';renderBossV2();renderBossHud();boss.raf=requestAnimationFrame(tickBoss)
  }
  ...
  function launchBossFx(ok,damage){
    ...
    shot.className=`boss-projectile ${ok?'player-strike':'boss-counter'}`;shot.textContent=ok?(bossConfig?.mechanic==='factor-shield'?'(x−a)':bossConfig?.mechanic==='conjugate-reflect'?'√±':pick(['f′','Δx','dy','∫'])):'×';
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',breakingShield=shieldBattle&&boss.shield>0,reflecting=reflectBattle&&boss.reflectCharge===1;
      if(breakingShield)boss.shield--;if(reflectBattle)boss.reflectCharge=reflecting?0:1;
      const hit=Math.max(1,Math.round((breakingShield?70:reflectBattle?(reflecting?bossConfig.baseDamage*2.15:85):bossConfig.baseDamage)*bossMultiplier()));
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
