(function(){
  ...
  const LEVEL_HP=bossConfig?.mechanic==='degree-grade'?[1,1.2,1.65]:[1,1,1];
  ...
  const bossStartCopy=()=>bossV2?`<strong>HP ${bossMaxHp()} · 첫 제한시간 ${bossBaseTime()}초</strong><p>${bossConfig.start}</p>`:'<strong>보스 체력 3칸</strong><p>정답은 한 칸 공격, 오답이나 시간 초과는 보스 체력을 전부 회복시킵니다.</p>';
  const bossDifficultyCopy=bossConfig?.mechanic==='degree-grade'?'난도 상승: <b>기본 → 응용 → 심화</b>':'선택 난이도 <b>고정</b> · 전투 단계만 상승';
  const initialBossStateLabel=bossConfig?.mechanic==='factor-shield'?'SHIELD':bossConfig?.mechanic==='conjugate-reflect'?'MIRROR':bossConfig?.mechanic==='degree-grade'?'GRADE':bossConfig?.mechanic==='chaos-split'?'BODIES':bossConfig?.mechanic==='side-switch'?'방향':bossConfig?.mechanic==='continuity-stitch'?'봉합':bossConfig?.mechanic==='squeeze-walls'?'벽 간격':bossConfig?.mechanic==='forbidden-seal'?'정석 봉인':bossConfig?.mechanic==='h-collapse'?'h 거리':bossConfig?.mechanic==='product-blades'?'쌍날':bossConfig?.mechanic==='sniper-lock'?'조준':'PHASE';
  const initialBossStateValue=bossConfig?.mechanic==='factor-shield'?3:bossConfig?.mechanic==='conjugate-reflect'?'0/2':bossConfig?.mechanic==='degree-grade'?'C':bossConfig?.mechanic==='side-switch'?'← 좌':bossConfig?.mechanic==='continuity-stitch'?'0/3':bossConfig?.mechanic==='squeeze-walls'?'100%':bossConfig?.mechanic==='forbidden-seal'?'3/3':bossConfig?.mechanic==='h-collapse'?'1':bossConfig?.mechanic==='product-blades'?'좌 대기':bossConfig?.mechanic==='sniper-lock'?'0/2':1;
  ...
  app.innerHTML=`<div class="calc-skill-page">
    ...
    <main class="skill-wrap">
      ...
      <section class="hidden" data-panel="boss"><header class="mode-header"><div><p class="skill-kicker">${bossV2?'BOSS BATTLE 2.2':'TIMED BOSS BATTLE'}</p><h1>${skill.title} 보스전</h1><p>${bossIntro}</p></div><div class="mode-stat"><span>CLEARS</span><strong data-boss-clears>0</strong></div></header><div class="boss-stage${bossV2?` boss-v2 boss-${bossConfig.theme}`:''}" data-boss-stage><div class="boss-arena"><div class="boss-fx-layer" data-boss-fx aria-hidden="true"></div><div class="boss-monster" data-boss-monster aria-label="${bossName}"><div class="boss-aura"></div>${bossVisual}<strong>${bossName}</strong><span data-boss-mood>도전자를 기다리는 중</span></div><div class="boss-dashboard"><div class="boss-hp-label"><span>BOSS HP</span><strong data-boss-hp-text>${bossV2?`${bossMaxHp()} / ${bossMaxHp()}`:'3 / 3'}</strong></div><div class="boss-hp-track"><div data-boss-hp></div></div><div class="boss-clock"><span>남은 시간</span><strong data-boss-time>${bossV2?`${bossBaseTime()}.0`:'12.0'}</strong><em>SEC</em></div><div class="boss-timer-track"><div data-boss-timer></div></div>${bossV2?`<div class="boss-combat-stats"><div><span>DAMAGE</span><b data-boss-damage>0</b></div><div><span>COMBO</span><b data-boss-combo>×1.0</b></div><div><span data-boss-state-label>${initialBossStateLabel}</span><b data-boss-phase>${initialBossStateValue}</b></div></div><small class="boss-limit-copy">${bossDifficultyCopy}</small>`:'<small class="boss-limit-copy">현재 문제당 <b data-boss-limit>12.0초</b> · 최소 7초</small>'}</div></div><div class="boss-progress"><i class="boss-node"></i><i class="boss-node"></i><i class="boss-node"></i></div><div data-boss-body><div class="boss-start-copy">${bossStartCopy()}</div></div><div class="boss-status hidden" data-boss-status></div><button class="primary-action boss-action" data-boss-start>⚔ ${bossV2?`${bossBaseTime()}초 전투 시작`:'보스전 시작'}</button></div></section>
    </main></div>`;
  ...
  const currentBossV2Limit=()=>Math.max(Math.round(BOSS_V2_MIN_TIME*LEVEL_TIME[bossLevelIndex()]),bossBaseTime()-(rec().bossClears||0)*2);
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse',productBattle=bossConfig.mechanic==='product-blades',sniperBattle=bossConfig.mechanic==='sniper-lock',wallGap=Number.isFinite(boss.wallGap)?boss.wallGap:100;$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':reflectBattle?'MIRROR':gradeBattle?'GRADE':chaosBattle?'BODIES':sideBattle?'방향':stitchBattle?'봉합':squeezeBattle?'벽 간격':forbiddenBattle?'정석 봉인':differenceBattle?'h 거리':productBattle?'쌍날':sniperBattle?'조준':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:reflectBattle?`${boss.reflectCharge}/2`:gradeBattle?boss.attackGrade:chaosBattle?boss.splitCount:sideBattle?(boss.targetSide==='right'?'우 →':'← 좌'):stitchBattle?`${boss.stitches}/3`:squeezeBattle?`${Math.round(wallGap)}%`:forbiddenBattle?`${boss.orthodoxSeal}/3`:differenceBattle?hDistanceLabel():productBattle?bladeStateLabel():sniperBattle?sniperLockLabel():boss.phase;
  ...
  function bossStartPhase(){return currentLevel==='deep'?3:currentLevel==='applied'?2:1}
  // 전투 PHASE와 학생이 고른 문제 난이도를 분리한다.
  // 미분의 철갑수만 "갑옷이 깨질수록 문제도 강해지는" 고유 능력을 유지한다.
  function bossQuestionLevel(){return bossConfig?.mechanic==='degree-grade'?(boss.phase===3?'deep':boss.phase===2?'applied':'basic'):currentLevel}
  ...
  /* 같은 곡선과 같은 a 로 세 문제를 잇는다. 세 번째 문제가 앞의 두
     답을 합치는 것이라, 목표가 중간에 바뀌면 이야기가 끊긴다. */
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit(),startPhase=bossStartPhase();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:bossMaxHp(),maxHp:bossMaxHp(),damage:0,combo:0,correct:0,attempts:0,phase:startPhase,shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,wallGap:100,wallCrushed:false,wallEscapes:0,orthodoxSeal:3,forbiddenPower:0,sealBreaks:0,resealOnNext:false,lastForbidden:false,hStep:0,hCoreHits:0,hResetOnNext:false,leftBladeReady:false,rightBladeReady:false,nextBlade:'left',bladePairs:0,bladesResetOnNext:false,productPair:null,sniperLock:0,sniperTarget:null,sniperShots:0,sniperResetOnNext:false,questionStartedAt:0,q:null,locked:false};
    ...
    const monster=$('[data-boss-monster]');monster.classList.remove('boss-hit','boss-attack','boss-defeated');$('[data-boss-mood]').textContent=bossConfig.mechanic==='factor-shield'?'석문 보호막 3칸':bossConfig.mechanic==='conjugate-reflect'?'거울 충전 0 / 2':bossConfig.mechanic==='degree-grade'?'하위 차수 장갑 · C GRADE':bossConfig.mechanic==='chaos-split'?'혼돈 코어 안정 · 1 BODY':bossConfig.mechanic==='side-switch'?'왼쪽 가면 봉인 · ← 좌극한':bossConfig.mechanic==='continuity-stitch'?'재생축 가동 · 봉합 0/3':bossConfig.mechanic==='squeeze-walls'?'상·하한 벽 전개 · 간격 100%':bossConfig.mechanic==='forbidden-seal'?'금단 두루마리 봉인 · 정석 0/3':bossConfig.mechanic==='h-collapse'?'할선 고리 전개 · h=1':bossConfig.mechanic==='sniper-lock'?'조준경 개방 · 조준 0/2':bossConfig.mechanic==='product-blades'?'청록 왼날 대기 · u′v':'기본 방어';if(startPhase>1)$('[data-boss-mood]').textContent+=` · ${LEVELS[startPhase-1].name} 시작`;$('[data-boss-start]').classList.add('hidden');$('[data-boss-status]').className='boss-status hidden';renderBossV2();renderBossHud();boss.raf=requestAnimationFrame(tickBoss)
  }
  ...
  function renderBossV2(){
    if(!boss.running)return;boss.locked=false;const level=bossQuestionLevel(),sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse',productBattle=bossConfig.mechanic==='product-blades',sniperBattle=bossConfig.mechanic==='sniper-lock';if(stitchBattle&&boss.reopenOnNext){boss.stitches=0;boss.reopenOnNext=false;boss.regenBuffer=0}if(squeezeBattle){boss.wallGap=100;boss.wallCrushed=false}if(forbiddenBattle&&boss.resealOnNext){boss.orthodoxSeal=3;boss.resealOnNext=false}if(differenceBattle&&boss.hResetOnNext){boss.hStep=0;boss.hResetOnNext=false}if(sniperBattle&&boss.sniperResetOnNext){boss.sniperLock=0;boss.sniperTarget=null;boss.sniperResetOnNext=false}if(productBattle&&boss.bladesResetOnNext){boss.leftBladeReady=false;boss.rightBladeReady=false;boss.nextBlade='left';boss.productPair=null;boss.bladesResetOnNext=false}$('[data-boss-stage]').classList.remove('boss-stitch-complete','boss-wall-slam','boss-seal-break','boss-forbidden-cast','boss-h-collapse','boss-h-rebound','boss-blade-cross','boss-blade-break','boss-sniper-fire','boss-sniper-miss');boss.q=sniperBattle?makeSniperQuestion(level):forbiddenBattle?makeForbiddenQuestion(level):differenceBattle?makeDifferenceQuestion(level):productBattle?makeProductBossQuestion(level,boss.nextBlade):makeQuestion(skill.id,level,sideBattle?boss.nextSide:null);if(sideBattle)boss.targetSide=boss.q.side||boss.nextSide;boss.attackGrade=level==='deep'?'A':level==='applied'?'B':'C';boss.questionStartedAt=performance.now();const levelName=LEVELS.find(l=>l.id===level).name,directionCue=sideBattle?` · ${boss.targetSide==='left'?'← 좌극한':'우극한 →'}`:stitchBattle?` · 금실 ${boss.stitches}/3`:squeezeBattle?` · 압착 ${squeezeSafeWindow().toFixed(1)}초`:forbiddenBattle?` · 정석 ${3-boss.orthodoxSeal}/3`:differenceBattle?` · h=${hDistanceLabel()}`:productBattle?` · ${boss.nextBlade==='left'?'청록 u′v':'금빛 uv′'}`:sniperBattle?` · 조준 ${sniperLockLabel()}`:'';const body=$('[data-boss-body]');body.innerHTML=`<div class="question-topline"><span>PHASE ${boss.phase} · ${levelName} 문제${directionCue}</span><span data-live-combo></span></div><div class="question-equation"></div><div class="question-prompt"></div><div class="answer-grid"></div>`;
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',reflectBattle=bossConfig.mechanic==='conjugate-reflect',gradeBattle=bossConfig.mechanic==='degree-grade',chaosBattle=bossConfig.mechanic==='chaos-split',sideBattle=bossConfig.mechanic==='side-switch',stitchBattle=bossConfig.mechanic==='continuity-stitch',squeezeBattle=bossConfig.mechanic==='squeeze-walls',forbiddenBattle=bossConfig.mechanic==='forbidden-seal',differenceBattle=bossConfig.mechanic==='h-collapse',productBattle=bossConfig.mechanic==='product-blades',sniperBattle=bossConfig.mechanic==='sniper-lock',breakingShield=shieldBattle&&boss.shield>0,reflecting=reflectBattle&&boss.reflectCharge===1,chaosLimit=boss.phase===3?3.5:boss.phase===2?4.1:4.8,fastFusion=chaosBattle&&answerSeconds<=chaosLimit,boundaryCrossing=sideBattle&&boss.lastSide&&boss.lastSide!==q.side,completingStitch=stitchBattle&&boss.stitches===2,withinSqueezeWindow=squeezeBattle&&!boss.wallCrushed&&answerSeconds<=squeezeSafeWindow(),breakingForbiddenSeal=forbiddenBattle&&boss.orthodoxSeal===1,exposingHCore=differenceBattle&&boss.hStep===2,completingProduct=productBattle&&q.blade==='right'&&boss.leftBladeReady,firingSniper=sniperBattle&&boss.sniperLock===2;
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:bossMaxHp(),maxHp:bossMaxHp(),damage:0,combo:0,correct:0,attempts:0,phase:bossStartPhase(),shield:bossConfig.mechanic==='factor-shield'?3:0,reflectCharge:0,attackGrade:'C',splitCount:1,nextSide:'left',targetSide:'left',lastSide:null,boundaryStreak:0,stitches:0,regenBuffer:0,healed:0,reopenOnNext:false,wallGap:100,wallCrushed:false,wallEscapes:0,orthodoxSeal:3,forbiddenPower:0,sealBreaks:0,resealOnNext:false,lastForbidden:false,hStep:0,hCoreHits:0,hResetOnNext:false,leftBladeReady:false,rightBladeReady:false,nextBlade:'left',bladePairs:0,bladesResetOnNext:false,productPair:null,sniperLock:0,sniperTarget:null,sniperShots:0,sniperResetOnNext:false,questionStartedAt:0,q:null,locked:false}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
