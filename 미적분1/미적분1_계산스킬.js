(function(){
  ...
  const BOSS_V2_HP=2600,BOSS_V2_BASE_TIME=38,BOSS_V2_MIN_TIME=24;
  const bossIntro=bossV2?'기본 → 응용 → 심화로 문제가 강해집니다. 클리어할수록 문제는 더 어려워지는 대신 다음 전투의 시간만 2초씩 줄어듭니다.':'세 번 공격해 보스의 체력을 모두 깎으세요. 클리어마다 문제 제한시간이 0.5초씩 줄어듭니다.';
  const bossStartCopy=bossV2?`<strong>HP ${BOSS_V2_HP} · 첫 제한시간 ${BOSS_V2_BASE_TIME}초</strong><p>정답은 수식 공격으로 바뀝니다. 3단계까지 문제 난도가 올라가며 오답은 콤보 초기화와 시간 −2초입니다.</p>`:'<strong>보스 체력 3칸</strong><p>정답은 한 칸 공격, 오답이나 시간 초과는 보스 체력을 전부 회복시킵니다.</p>';
  const bossVisual=bossV2?'<div class="boss-art-frame"><img class="boss-art" src="../assets/bosses/derivative-iron-beast.webp" alt="곡선 갑옷과 빛나는 미분 코어를 지닌 미분의 철갑수"></div>':'<svg class="boss-creature" viewBox="0 0 240 190" role="img" aria-hidden="true"><path class="boss-wing" d="M73 73C36 44 16 62 27 91c8 22 29 30 52 27M167 73c37-29 57-11 46 18-8 22-29 30-52 27"/><path class="boss-horn" d="M86 50 63 16l42 23M154 50l23-34-42 23"/><path class="boss-body-shape" d="M120 34c-45 0-70 31-64 75 5 39 27 64 64 64s59-25 64-64c6-44-19-75-64-75Z"/><path class="boss-mask" d="M76 78c29-17 59-17 88 0l-11 47c-22 17-44 17-66 0Z"/><path class="boss-eye" d="m88 87 23 8-25 7Zm64 0-23 8 25 7Z"/><path class="boss-core" d="m120 117 13 18-13 15-13-15Z"/></svg>';
  ...
  app.innerHTML=`<div class="calc-skill-page">
    ...
    <main class="skill-wrap">
      ...
      <section class="hidden" data-panel="boss"><header class="mode-header"><div><p class="skill-kicker">${bossV2?'BOSS BATTLE 2.1':'TIMED BOSS BATTLE'}</p><h1>${skill.title} 보스전</h1><p>${bossIntro}</p></div><div class="mode-stat"><span>CLEARS</span><strong data-boss-clears>0</strong></div></header><div class="boss-stage${bossV2?' boss-v2':''}" data-boss-stage><div class="boss-arena"><div class="boss-fx-layer" data-boss-fx aria-hidden="true"></div><div class="boss-monster" data-boss-monster aria-label="${bossName}"><div class="boss-aura"></div>${bossVisual}<strong>${bossName}</strong><span data-boss-mood>도전자를 기다리는 중</span></div><div class="boss-dashboard"><div class="boss-hp-label"><span>BOSS HP</span><strong data-boss-hp-text>${bossV2?`${BOSS_V2_HP} / ${BOSS_V2_HP}`:'3 / 3'}</strong></div><div class="boss-hp-track"><div data-boss-hp></div></div><div class="boss-clock"><span>남은 시간</span><strong data-boss-time>${bossV2?`${BOSS_V2_BASE_TIME}.0`:'12.0'}</strong><em>SEC</em></div><div class="boss-timer-track"><div data-boss-timer></div></div>${bossV2?'<div class="boss-combat-stats"><div><span>DAMAGE</span><b data-boss-damage>0</b></div><div><span>COMBO</span><b data-boss-combo>×1.0</b></div><div><span>PHASE</span><b data-boss-phase>1</b></div></div><small class="boss-limit-copy">난도 상승: <b>기본 → 응용 → 심화</b></small>':'<small class="boss-limit-copy">현재 문제당 <b data-boss-limit>12.0초</b> · 최소 7초</small>'}</div></div><div class="boss-progress"><i class="boss-node"></i><i class="boss-node"></i><i class="boss-node"></i></div><div data-boss-body><div class="boss-start-copy">${bossStartCopy}</div></div><div class="boss-status hidden" data-boss-status></div><button class="primary-action boss-action" data-boss-start>⚔ ${bossV2?`${BOSS_V2_BASE_TIME}초 전투 시작`:'보스전 시작'}</button></div></section>
    </main></div>`;
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:1,q:null,locked:false};
    ...
    const monster=$('[data-boss-monster]');monster.classList.remove('boss-hit','boss-attack','boss-defeated');$('[data-boss-mood]').textContent='기본 방어 · PHASE 1';$('[data-boss-start]').classList.add('hidden');$('[data-boss-status]').className='boss-status hidden';renderBossV2();renderBossHud();boss.raf=requestAnimationFrame(tickBoss)
  }
  ...
  function renderBossV2(){
    if(!boss.running)return;boss.locked=false;boss.q=makeQuestion(skill.id,bossQuestionLevel());const levelName=LEVELS.find(l=>l.id===bossQuestionLevel()).name;const body=$('[data-boss-body]');body.innerHTML=`<div class="question-topline"><span>PHASE ${boss.phase} · ${levelName} 문제</span><span data-live-combo></span></div><div class="question-equation"></div><div class="question-prompt"></div><div class="answer-grid"></div>`;
  ...
  function launchBossFx(ok,damage){
    const layer=$('[data-boss-fx]');if(!layer)return;
    const shot=document.createElement('i'),impact=document.createElement('b');
    shot.className=`boss-projectile ${ok?'player-strike':'boss-counter'}`;shot.textContent=ok?pick(['f′','Δx','dy','∫']):'×';
    impact.className=`boss-impact ${ok?'player-impact':'counter-impact'}`;impact.textContent=ok?`−${damage}`:'−2초';
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){boss.combo++;boss.correct++;const hit=Math.max(1,Math.round(170*bossMultiplier()));boss.hp=Math.max(0,boss.hp-hit);boss.damage+=hit;launchBossFx(true,hit);monster.classList.remove('boss-attack');monster.classList.add('boss-hit');st.textContent=`${hit} DAMAGE · ${q.explanation}`;st.className='boss-status good';const ratio=boss.hp/boss.maxHp,nextPhase=ratio<=.34?3:ratio<=.67?2:1;if(nextPhase!==boss.phase){boss.phase=nextPhase;$('[data-boss-mood]').textContent=`${nextPhase===2?'응용 장갑 전개':'심화 코어 폭주'} · PHASE ${nextPhase}`}else $('[data-boss-mood]').textContent=`${boss.combo} COMBO · 공격 적중`;if(boss.hp<=0)cancelAnimationFrame(boss.raf);renderBossHud();setTimeout(()=>{monster.classList.remove('boss-hit');if(!boss.running)return;if(boss.hp<=0)finishBossV2(true);else{st.className='boss-status hidden';renderBossV2()}},620)}
    else{boss.combo=0;boss.time=Math.max(0,boss.time-2);launchBossFx(false,0);monster.classList.remove('boss-hit');monster.classList.add('boss-attack');$('[data-boss-mood]').textContent='철갑 반격 · COMBO RESET';st.textContent=`오답 · 시간 −2초 · ${q.explanation}`;st.className='boss-status bad';renderBossHud();setTimeout(()=>{monster.classList.remove('boss-attack');if(!boss.running)return;st.className='boss-status hidden';renderBossV2()},760)}
  }
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,phase:1}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
