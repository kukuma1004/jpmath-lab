(function(){
  ...
  function renderBossHud(){
    ...
    if(bossV2){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield';$('[data-boss-state-label]').textContent=shieldBattle?'SHIELD':'PHASE';$('[data-boss-phase]').textContent=shieldBattle?boss.shield:boss.phase;
  ...
  function startBossV2(){
    stopBossTimer();bossSessionPlayIndex++;const limit=currentBossV2Limit();boss={running:true,raf:0,time:limit,limit,last:performance.now(),hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,attempts:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0,q:null,locked:false};
    ...
    const monster=$('[data-boss-monster]');monster.classList.remove('boss-hit','boss-attack','boss-defeated');$('[data-boss-mood]').textContent=bossConfig.mechanic==='factor-shield'?'석문 보호막 3칸':'기본 방어 · PHASE 1';$('[data-boss-start]').classList.add('hidden');$('[data-boss-status]').className='boss-status hidden';renderBossV2();renderBossHud();boss.raf=requestAnimationFrame(tickBoss)
  }
  ...
  function resolveBossV2(value,button){
    ...
    if(ok){
      ...
      const shieldBattle=bossConfig.mechanic==='factor-shield',breakingShield=shieldBattle&&boss.shield>0;
      if(breakingShield)boss.shield--;
      const hit=Math.max(1,Math.round((breakingShield?70:bossConfig.baseDamage)*bossMultiplier()));
      ...
      st.textContent=breakingShield?`보호막 파괴 · ${boss.shield}칸 남음 · ${q.explanation}`:`${hit} DAMAGE · ${q.explanation}`;st.className='boss-status good';
      const ratio=boss.hp/boss.maxHp,nextPhase=ratio<=.34?3:ratio<=.67?2:1;
      if(nextPhase!==boss.phase){boss.phase=nextPhase;if(shieldBattle){boss.shield=nextPhase===2?2:1;$('[data-boss-mood]').textContent=`석문 재결합 · 보호막 ${boss.shield}칸`}else $('[data-boss-mood]').textContent=`${nextPhase===2?'응용 장갑 전개':'심화 코어 폭주'} · PHASE ${nextPhase}`}
  ...
  function resetModes(){
    ...
    if(bossV2){const limit=currentBossV2Limit();boss={running:false,raf:0,time:limit,limit,hp:BOSS_V2_HP,maxHp:BOSS_V2_HP,damage:0,combo:0,correct:0,phase:1,shield:bossConfig.mechanic==='factor-shield'?3:0}}else{boss.index=0;boss.limit=currentBossLimit();boss.time=boss.limit}renderBossHud();
  }
