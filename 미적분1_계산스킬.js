(function(){
  ...
  const skillList=[
    ...
    S('squeeze_limit','L07','샌드위치 정리 계산','양쪽에서 끼워 극한을 결정','limit','확장','|g(x)|≤h(x), h(x)→0 ⇒ g(x)→0',['진동하는 부분의 절댓값 범위 찾기','전체 식을 0과 쉬운 식 사이에 끼우기','양쪽 극한이 같음을 확인'],'sin(1/x) 자체의 극한은 없지만 곱 전체는 존재할 수 있습니다.'),
    S('lhopital','L08','로피탈 정리','미정형에서 분자·분모를 각각 미분','limit','대학 맛보기','0/0 또는 ∞/∞에서 lim f/g = lim f′/g′',['먼저 미정형 조건 확인','분자와 분모를 따로 미분','새 극한을 계산하고 필요하면 반복'],'곱이나 차에 바로 로피탈을 적용할 수 없습니다.'),
    S('derivative_definition','D09','미분계수의 정의','차분몫의 극한을 도함수 값으로 연결','differentiate','교과 핵심',"f′(a)=lim[h→0]{f(a+h)−f(a)}/h",['분자에 f(a+h)와 f(a)를 정확히 대입','전개 후 h를 공통인수로 묶기','h를 약분한 뒤 h→0'],'분모 h를 약분하기 전에 h=0을 대입하지 마세요.'),
    S('differentiate_polynomial','D10','다항함수 빠른 미분','계수×지수, 지수는 하나 내리기','differentiate','교과 핵심',"(axⁿ)′=anxⁿ⁻¹",['각 항을 따로 미분','계수에 기존 지수를 곱하기','상수항은 0으로 처리'],'지수만 내리고 계수에 곱하지 않는 실수를 조심하세요.'),
    S('product_rule','D11','곱의 미분법','앞미분×뒤 + 앞×뒤미분','differentiate','수능 실전',"(fg)′=f′g+fg′",['두 함수를 앞과 뒤로 구분','앞만 미분한 항 작성','뒤만 미분한 항을 더하기'],'두 함수를 동시에 미분한 f′g′가 아닙니다.'),
    S('quotient_rule','D12','몫의 미분법','분모 제곱과 분자 순서를 고정','differentiate','확장',"(f/g)′=(f′g−fg′)/g²",['분모를 먼저 제곱해 틀 만들기','분자는 f′g−fg′ 순서','마지막에 약분 가능성 확인'],'분자의 뺄셈 순서를 바꾸면 부호가 반대가 됩니다.'),
    S('chain_rule','D13','합성함수 연쇄법칙','바깥 미분×안쪽 미분','differentiate','확장',"{F(g(x))}′=F′(g(x))g′(x)",['가장 바깥 함수부터 미분','안쪽 식은 그대로 유지','안쪽 함수의 미분을 곱하기'],'안쪽 함수의 미분을 빠뜨리지 마세요.'),
    S('tangent_equation','D14','접선의 방정식','기울기와 접점을 한 번에 대입','differentiate','교과 핵심',"y−f(a)=f′(a)(x−a)",['접점의 y좌표 f(a) 계산','접선 기울기 f′(a) 계산','점-기울기 공식에 대입'],'접점의 x좌표 a를 기울기로 착각하지 마세요.'),
    S('higher_derivative','D15','고계도함수','반복 미분과 계승 패턴','differentiate','대학 맛보기',"(xⁿ)⁽ⁿ⁾=n!",['한 번 미분할 때마다 지수 내리기','앞에 곱해지는 수를 누적','필요한 미분 차수에서 멈추기'],'n차 미분 뒤에는 상수가 되고 다음 미분은 0입니다.'),
    S('implicit_derivative','D16','음함수 미분','x와 y가 섞인 식에서 dy/dx 찾기','differentiate','대학 맛보기',"x²+y²=r² ⇒ y′=−x/y",['식의 양변을 x로 미분','y항에는 y′를 함께 곱하기','y′가 있는 항을 모아 풀기'],'y를 x처럼 미분해 2y로 끝내면 안 됩니다.'),
    S('monotonic_interval','G17','증가·감소 구간','도함수의 부호표를 구간으로 읽기','graph','교과 핵심',"f′>0 증가, f′<0 감소",['도함수의 근으로 수직선을 분할','각 구간에서 도함수 부호 판정','양수 구간과 음수 구간을 기록'],'f의 부호가 아니라 f′의 부호를 봅니다.'),
    S('extrema_sign','G18','극대·극소 부호 변화','+→−와 −→+를 즉시 판정','graph','교과 핵심',"+→− 극대, −→+ 극소",['임계점 왼쪽 부호 확인','오른쪽 부호 확인','증가·감소의 전환으로 이름 결정'],'f′(a)=0만으로 극값이라고 단정할 수 없습니다.'),
    S('cubic_extrema','G19','삼차함수의 극값 조건',"f′의 판별식으로 개형 결정",'graph','수능 실전','ax³+bx²+cx+d: b²−3ac>0',['삼차함수를 한 번 미분','이차 도함수의 판별식 계산','서로 다른 두 실근 조건 D>0 적용'],'D=0이면 수평접선은 있지만 극값은 없습니다.'),
    S('quartic_shape','G20','사차함수와 도함수','세 임계점의 극대·극소 배열','graph','수능 실전',"f′의 부호: − + − + ⇒ 극소·극대·극소",['삼차 도함수의 실근을 순서대로 표시','각 구간의 부호를 번갈아 기록','원함수의 오르내림으로 개형 결정'],'도함수의 그래프와 원함수의 그래프를 혼동하지 마세요.'),
    S('real_roots','G21','방정식의 실근 개수','그래프와 수평선의 교점 세기','graph','수능 실전','f(x)=k의 실근 수 = y=f(x)와 y=k의 교점 수',['함수의 극댓값과 극솟값 확인','수평선 y=k의 높이 비교','접하는 경우 중근까지 포함해 서로 다른 근 세기'],'중근은 교점 하나이므로 서로 다른 실근은 한 개로 셉니다.'),
    S('mean_value','G22','평균값 정리·롤의 정리','평균변화율과 같은 순간기울기','graph','교과 핵심',"f′(c)={f(b)−f(a)}/{b−a}",['구간에서 연속·내부에서 미분가능 확인','양 끝점으로 평균변화율 계산','f′(c)와 같게 놓아 c 찾기'],'c는 반드시 열린구간 (a,b) 안에 있어야 합니다.'),
    S('motion_rate','G23','속도와 가속도','위치→속도→가속도 순서','graph','교과 핵심',"v(t)=s′(t), a(t)=v′(t)",['위치함수를 한 번 미분해 속도','속도를 한 번 더 미분해 가속도','묻는 시각을 마지막에 대입'],'속력은 |v|이고 속도 v와 구분해야 합니다.'),
    S('horizontal_tangent','G24','수평접선과 접선 개수','도함수의 근을 그래프 조건으로 변환','graph','수능 실전',"수평접선 ⇔ f′(x)=0",['접선의 기울기 조건을 도함수 식으로 변환','도함수 방정식의 서로 다른 실근 계산','각 근이 실제 접점이 되는지 확인'],'중근에서는 수평접선이 하나만 생깁니다.'),
    S('antiderivative','I25','부정적분 기본','지수를 하나 올리고 새 지수로 나누기','integral','교과 핵심','∫xⁿdx=xⁿ⁺¹/(n+1)+C',['각 항의 지수를 하나 올리기','올라간 새 지수로 계수 나누기','마지막에 적분상수 C 붙이기'],'부정적분에서 +C를 빠뜨리지 마세요.'),
    S('initial_antiderivative','I26','조건으로 원함수 결정','적분상수 C를 주어진 값으로 결정','integral','교과 핵심',"F′=f, F(a)=b로 C 결정",['도함수를 부정적분','원함수에 조건의 x값 대입','등식으로 적분상수 C 계산'],'조건을 적용하기 전에 C를 임의로 0으로 두지 마세요.'),
    S('definite_integral','I27','정적분 계산','원시함수에 위끝−아래끝 대입','integral','교과 핵심','∫[a→b]f(x)dx=F(b)−F(a)',['먼저 원시함수 F 찾기','위끝값 F(b) 계산','아래끝값 F(a)를 빼기'],'아래끝값 앞의 마이너스를 분배하세요.'),
    S('integral_symmetry','I28','정적분의 대칭성','홀함수·짝함수를 구간과 함께 판정','integral','수능 실전','홀함수: ∫[−a→a]f=0',['적분구간이 원점 대칭인지 확인','함수가 홀함수인지 짝함수인지 판정','홀함수면 0, 짝함수면 두 배'],'함수의 대칭성과 구간의 대칭이 모두 필요합니다.'),
    S('area_axis','I29','곡선과 x축 사이 넓이','부호가 바뀌는 지점에서 구간 분할','integral','교과 핵심','넓이=∫|f(x)|dx',['x축과 만나는 점을 먼저 찾기','부호가 바뀌는 곳에서 구간 나누기','음수 구간 적분값은 부호를 바꾸어 더하기'],'정적분값과 넓이는 음수 구간에서 다릅니다.'),
    S('area_between','I30','두 곡선 사이 넓이','위 함수−아래 함수의 정적분','integral','수능 실전','넓이=∫|f(x)−g(x)|dx',['두 곡선의 교점으로 구간 확인','각 구간에서 위아래 함수 판정','위−아래를 적분해 양수 넓이로 합하기'],'그래프의 위아래가 바뀌면 식의 순서도 바뀝니다.'),
    S('distance_velocity','I31','속도에서 이동거리','속도의 부호가 바뀌면 절댓값 적분','integral','수능 실전','이동거리=∫|v(t)|dt',['v(t)=0인 시각 찾기','속도의 부호가 바뀌는 구간 분할','각 구간의 변위를 절댓값으로 더하기'],'이동거리는 전체 변위의 절댓값과 다를 수 있습니다.'),
    S('fundamental_theorem','I32','미적분의 기본정리','적분으로 정의된 함수의 미분','integral','확장',"d/dx ∫[a→x]f(t)dt=f(x)",['적분의 위끝이 x인지 확인','적분 안의 변수 t를 그대로 읽기','위끝이 g(x)이면 연쇄법칙까지 곱하기'],'적분 변수 t와 바깥 변수 x를 혼동하지 마세요.')
  ];
  ...
  function makeQuestion(id){
    ...
    switch(id){
      ...
      case'squeeze_limit':
        n=pick([2,4,6]);
        return Q('SQUEEZE',`lim x→0  x${pow('',n).replace('','')} sin(1/x)`,'극한값은?',0,[1,-1,'존재하지 않음'],`|x${['','','²','³','⁴','⁵','⁶'][n]}sin(1/x)|≤|x|${['','','²','³','⁴','⁵','⁶'][n]}이고 오른쪽이 0으로 갑니다.`);
      case'lhopital':
        k=ri(1,6);mode=pick(['sin','exp']);
        return mode==='sin'?Q('L’HÔPITAL',`lim x→0  sin(${k}x)/x`,'로피탈 정리를 적용한 극한값은?',k,[1,0,-k],`분자와 분모를 각각 미분하면 ${k}cos(${k}x)/1이므로 ${k}입니다.`):Q('L’HÔPITAL',`lim x→0  (e^(${k}x)−1)/x`,'로피탈 정리를 적용한 극한값은?',k,[1,0,-k],`분자와 분모를 각각 미분하면 ${k}e^(${k}x)/1이므로 ${k}입니다.`);
      ...
      case'quotient_rule':
        A=ri(1,5);B=ri(1,6);c=ri(1,5);correct=frac(A*c-B,c*c);
        return Q('QUOTIENT RULE',`f(x)=(${A}x+${B})/(x+${c})`,'f′(0)의 값은?',correct,[frac(A*c+B,c*c),frac(A-B,c),String(A)],`f′(x)={${A}(x+${c})−(${A}x+${B})}/(x+${c})²이므로 ${correct}입니다.`);
      case'chain_rule':
        A=ri(1,3);B=ri(1,3);n=ri(2,4);correct=n*A*Math.pow(B,n-1);
        return Q('CHAIN RULE',`f(x)=(${A}x+${B})${['','¹','²','³','⁴'][n]}`,'f′(0)의 값은?',correct,[n*Math.pow(B,n-1),A*Math.pow(B,n),n*A*B],`바깥 미분에 안쪽 미분 ${A}를 곱하면 ${correct}입니다.`);
      ...
      case'higher_derivative':
        A=ri(1,3);n=ri(2,5);correct=A*factorial(n);
        return Q('HIGHER DERIVATIVE',`f(x)=${A}${pow('x',n)}`,'f의 '+n+'차 도함수는?',correct,[factorial(n),A*n,A*factorial(n-1)],`${pow('x',n)}을 ${n}번 미분하면 ${n}!=${factorial(n)}이므로 ${correct}입니다.`);
      case'implicit_derivative':
        [a,b,c]=pick([[3,4,5],[4,3,5],[5,12,13],[8,15,17]]);correct=frac(-a,b);
        return Q('IMPLICIT',`x²+y²=${c*c},  P(${a},${b})`,'점 P에서 dy/dx는?',correct,[frac(a,b),frac(-b,a),frac(b,a)],`2x+2yy′=0이므로 y′=−x/y=${correct}입니다.`);
  ...
  const params=new URLSearchParams(location.search),skill=skills[params.get('id')];
  const app=$('#calcSkillApp');
  if(!skill){app.innerHTML='<div class="not-found"><h1>계산 스킬을 찾을 수 없어요.</h1><p>미적분 스킬 지도에서 다시 선택해 주세요.</p><a href="미적분1.html#skills">스킬 지도로 돌아가기</a></div>';return}
