(function(){
  'use strict';
  const commonScriptUrl=document.currentScript&&document.currentScript.src?document.currentScript.src:location.href;
  function loadScriptOnce(src,marker){
    if(marker&&window[marker])return Promise.resolve();
    const found=[...document.scripts].find(script=>script.src===src);
    if(found)return new Promise(resolve=>{if(marker&&window[marker])resolve();else{found.addEventListener('load',resolve,{once:true});found.addEventListener('error',resolve,{once:true})}});
    return new Promise(resolve=>{const script=document.createElement('script');script.src=src;script.addEventListener('load',resolve,{once:true});script.addEventListener('error',resolve,{once:true});document.head.appendChild(script)});
  }
  const telemetryReady=(async()=>{
    if(window.JPGameTelemetry)return window.JPGameTelemetry;
    await loadScriptOnce(new URL('../경제수학/live/firebase-config.js',commonScriptUrl).href,'JPEconomyFirebaseConfig');
    await loadScriptOnce(new URL('../jp-game-telemetry.js?v=2',commonScriptUrl).href,'JPGameTelemetry');
    return window.JPGameTelemetry||null;
  })();
  ...
  class Arena{
    constructor(root,config){this.root=root;this.c=config;this.mode='solo';this.seed=0;this.rounds=[];this.session=0;this.results=[];this.timer=0;this.locked=false;this.audio=null;this.lastTick=-1;this.telemetry=null;this.telemetrySessionId=`session-${Date.now()}-${config.id}`;this.sessionPlayIndex=0;this.activePlay=null;this.playStartedAt=0;this.enhanceMarkup();this.bind();this.prepareTelemetry();this.show('start');this.renderLeaders()}
    q(sel){return this.root.querySelector(sel)}
    enhanceMarkup(){
      const p1=this.q('[data-player-one]'),p2=this.q('[data-player-two]');
      if(p1){p1.value='';p1.maxLength=12;p1.placeholder='이름 입력';p1.autocomplete='nickname';const label=p1.closest('.name-field')?.querySelector('label');if(label)label.textContent='나의 이름';if(!this.q('[data-name-error]')){const error=document.createElement('small');error.className='name-error';error.dataset.nameError='';error.setAttribute('aria-live','polite');p1.closest('.name-field')?.appendChild(error)}p1.addEventListener('input',()=>{const error=this.q('[data-name-error]');if(error)error.textContent=''})}
      if(p2){p2.maxLength=12;p2.placeholder='친구 이름';const label=p2.closest('.name-field')?.querySelector('label');if(label)label.textContent='1:1 상대 이름'}
      const names=this.q('.name-grid');if(names&&!this.q('.player-memory')){const note=document.createElement('p');note.className='player-memory';note.textContent='한 번 입력하면 다른 JP Math Lab 게임에서도 같은 이름을 사용합니다.';names.insertAdjacentElement('afterend',note)}
      const solo=this.q('[data-solo-result]');
      if(solo&&!this.q('[data-final-accuracy]')){
        const badge=document.createElement('div');badge.className='result-badge hidden';badge.dataset.pbBadge='';badge.textContent='★ PERSONAL BEST';solo.insertBefore(badge,solo.firstChild);
        solo.insertAdjacentHTML('beforeend','<div class="result-stats"><article><span>정확도</span><strong data-final-accuracy>0%</strong></article><article><span>개인 최고</span><strong data-final-best>0</strong></article><article><span>이번 세션</span><strong data-final-play-index>1번째</strong></article></div><p class="result-feedback" data-final-feedback></p>');
      }
      const replay=this.q('[data-arena-replay]');
      if(replay&&!this.q('[data-change-mode]')){replay.textContent='바로 다시하기';const wrap=document.createElement('div');wrap.className='result-actions';replay.parentNode.insertBefore(wrap,replay);wrap.appendChild(replay);const change=document.createElement('button');change.type='button';change.className='btn btn-block';change.dataset.changeMode='';change.textContent='선수·모드 바꾸기';wrap.appendChild(change)}
    }
    async prepareTelemetry(){if(this.telemetry)return this.telemetry;if(this.telemetryPromise)return this.telemetryPromise;this.telemetryPromise=telemetryReady.then(telemetry=>{this.telemetry=telemetry;if(!telemetry)return null;const profile=telemetry.getProfile();if(profile&&!this.q('[data-player-one]').value)this.q('[data-player-one]').value=profile.displayName;this.telemetrySessionId=telemetry.makeSessionId();telemetry.flush();return telemetry});return this.telemetryPromise}
    async ensureProfile(){if(!this.telemetry)await this.prepareTelemetry();const input=this.q('[data-player-one]'),current=this.telemetry?this.telemetry.getProfile():null;if(!input.value.trim()&&current)input.value=current.displayName;const name=String(input.value||'').trim();if(!name){const error=this.q('[data-name-error]');if(error)error.textContent='기록을 남길 이름을 먼저 입력해 주세요.';input.focus();return false}this.profile=this.telemetry?this.telemetry.saveProfile(name):{userId:`local-${name}`,displayName:name};input.value=this.profile.displayName;return true}
    bind(){
      ...
      this.q('[data-arena-replay]').addEventListener('click',()=>this.start(this.mode));
      this.q('[data-change-mode]').addEventListener('click',()=>this.show('start'));
    ...
    show(name){this.root.querySelectorAll('[data-arena-screen]').forEach(n=>n.classList.toggle('hidden',n.dataset.arenaScreen!==name));if(name==='play'||name==='end'||name==='handoff')requestAnimationFrame(()=>window.scrollTo(0,0))}
    async start(mode){if(!(await this.ensureProfile()))return;this.mode=mode;if(mode==='solo'){this.sessionPlayIndex++;this.playStartedAt=Date.now();this.activePlay=this.telemetry?this.telemetry.startPlay({gameId:`geometry-${this.c.id}`,sessionId:this.telemetrySessionId,retry:this.sessionPlayIndex>1,sessionPlayIndex:this.sessionPlayIndex}):null}else this.activePlay=null;this.seed=Date.now()&0xffffffff;this.rounds=this.c.createRounds(seeded(this.seed));this.results=[];this.session=0;this.sound=this.q('[data-arena-sound]').checked;this.startSession(0)}
    ...
    finishSession(){this.stopTimer();this.results[this.session]={name:this.playerName(),score:this.state.score,correct:this.state.correct,answered:Math.min(this.state.round,this.rounds.length)};this.saveLeader(this.results[this.session]);if(this.mode==='duel'&&this.session===0){this.q('[data-handoff-name]').textContent=this.playerName(1);this.q('[data-handoff-score]').textContent=`${this.results[0].name}의 기록은 ${this.results[0].score}점. 같은 문제에 도전하세요.`;this.show('handoff')}else this.finishGame()}
    finishGame(){this.show('end');const solo=this.mode==='solo',r0=this.results[0];this.q('[data-solo-result]').classList.toggle('hidden',!solo);this.q('[data-duel-result]').classList.toggle('hidden',solo);if(solo){r0.accuracy=Math.round(r0.correct/Math.max(1,r0.answered)*100);const oldBest=this.loadPersonalBest(),personalBest=!oldBest||r0.score>Number(oldBest.score||0);if(personalBest)this.savePersonalBest(r0);const best=personalBest?r0:this.loadPersonalBest();this.q('[data-final-score]').textContent=r0.score;this.q('[data-final-summary]').textContent=`${r0.answered}문제 중 ${r0.correct}개 정답`;this.q('[data-final-accuracy]').textContent=`${r0.accuracy}%`;this.q('[data-final-best]').textContent=best.score;this.q('[data-final-play-index]').textContent=`${this.sessionPlayIndex}번째`;this.q('[data-pb-badge]').classList.toggle('hidden',!personalBest);this.q('[data-final-feedback]').textContent=personalBest?(oldBest?`${r0.score}점으로 개인 최고 기록을 세웠습니다. 공간의 관계를 더 빠르게 읽어 보세요.`:`${r0.score}점으로 첫 기록을 남겼습니다. 방금 본 공간을 기억하고 다시 도전해 보세요.`):(r0.accuracy>=75?`정확도 ${r0.accuracy}%. 속도를 조금만 높이면 개인 최고 ${best.score}점을 넘을 수 있어요.`:'도형을 머릿속에서 한 번 더 움직이며 바로 다시 도전해 보세요.');if(this.telemetry&&this.activePlay)this.telemetry.finishPlay(this.activePlay.playId,{endedAt:new Date().toISOString(),score:r0.score,accuracy:r0.accuracy,playTime:Math.max(1,Math.round((Date.now()-this.playStartedAt)/1000)),retry:this.sessionPlayIndex>1,sessionPlayIndex:this.sessionPlayIndex,personalBest});if(window.jpMotionFeedback)window.jpMotionFeedback('success',personalBest?`개인 최고 ${r0.score}점!`:`이번 기록 ${r0.score}점`)}else{const a=this.results[0],b=this.results[1];this.q('[data-duel-one-name]').textContent=a.name;this.q('[data-duel-one-score]').textContent=a.score;this.q('[data-duel-two-name]').textContent=b.name;this.q('[data-duel-two-score]').textContent=b.score;const one=this.q('[data-duel-one]'),two=this.q('[data-duel-two]');one.classList.toggle('winner',a.score>b.score);two.classList.toggle('winner',b.score>a.score);this.q('[data-winner-line]').textContent=a.score===b.score?'완벽한 무승부!':`${a.score>b.score?a.name:b.name} 승리!`};this.q('[data-arena-replay]').textContent=solo?'바로 다시하기':'바로 다시 대결';this.renderLeaders()}
    key(){return'jp_geo_arena_'+this.c.id}
    personalBestKey(){const userId=(this.profile&&this.profile.userId)||this.playerName();return`jp_geo_personal_best_${this.c.id}_${String(userId).replace(/[^a-zA-Z0-9가-힣_-]/g,'_')}`}
    loadPersonalBest(){try{const value=JSON.parse(localStorage.getItem(this.personalBestKey()));return value&&Number.isFinite(Number(value.score))?value:null}catch(e){return null}}
    savePersonalBest(result){try{localStorage.setItem(this.personalBestKey(),JSON.stringify({score:result.score,accuracy:result.accuracy,savedAt:new Date().toISOString()}))}catch(e){}}
