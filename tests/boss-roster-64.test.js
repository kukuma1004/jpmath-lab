...
assert.equal(playable.length,2,'현재 실제 플레이 가능한 보스는 두 종이어야 한다.');
assert.deepEqual([...playable.map(x=>x.name)].sort(),['미분의 철갑수','인수분해의 문지기'].sort());
...
assert.match(hall,/boss-catalog\.js\?v=2/);
