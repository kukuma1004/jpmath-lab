...
assert.match(skillHtml, /미적분1_계산스킬\.js\?v=19/);
assert.match(skillHtml, /미적분1_계산스킬\.css\?v=22/);
...
assert.match(skillJs, /requiresFinisher=stitchBattle&&!completingStitch\|\|forbiddenBattle&&!breakingForbiddenSeal\|\|differenceBattle&&!exposingHCore/,'완성 공격이 필요한 보스는 고유 규칙을 충족해야만 쓰러져야 한다.');
...
assert.match(skillJs, /String\(value\)==='로피탈 정리'/,'로피탈 정리를 실제로 선택했을 때만 금단 마력이 강화되어야 한다.');
