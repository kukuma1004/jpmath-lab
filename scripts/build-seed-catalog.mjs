...
loadBrowserData('seed-expand.js', context);

// 브라우저가 확장 씨앗을 만들 때 쓰는 바로 그 코드다.
// 아래에서 생성기가 만든 것과 글자 하나까지 같은지 맞춰 본다.
const expander = context.window.JPSeedExpand;
...
for (let lensIndex = 0; lensIndex < LENSES.length; lensIndex += 1) {
  ...
  for (const base of baseObjects) {
    ...
    baseIndexById.set(id, baseObjects.indexOf(base));
    generatedPool.push({
      id,
      src: 'seed2000',
      grade: 'B',
...
// 확장 씨앗은 파일에 담아 보내지 않는다. 브라우저가 BASES 와 picks 로
// 다시 만든다. 그래서 같은 코드로 여기서도 만들어 보고, 하나라도
// 다르면 멈춘다. 두 쪽이 어긋난 채 배포되는 일을 막는다.
const picks = generated.map((seed) => [baseIndexById.get(seed.id), seed.lens]);
const rebuilt = expander.expand(BASES, picks);
if (rebuilt.length !== generated.length) {
  throw new Error(`Rebuild count mismatch: ${rebuilt.length} vs ${generated.length}`);
}
for (let i = 0; i < generated.length; i += 1) {
  const a = { ...generated[i] };
  const b = { ...rebuilt[i] };
  delete b.stage; delete b.track;   // 분류는 아래에서 함께 붙인다
  if (JSON.stringify(a) !== JSON.stringify(b)) {
    throw new Error(`Rebuild mismatch at ${generated[i].id}`);
  }
}
...
const js = `/* AUTO-GENERATED by scripts/build-seed-catalog.mjs\n` +
  ...
  `   확장 ${catalog.length - curated.length}개는 담지 않는다. BASES 200개와 picks 로\n` +
  `   seed-expand.js 가 브라우저에서 만든다. 그래서 파일이 작다.\n` +
  ...
  `  var catalog = curated.concat(window.JPSeedExpand.expand(BASES, picks));\n` +
